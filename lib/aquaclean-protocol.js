...
const AQUACLEAN_COMMANDS = Object.freeze({
  ...
  // Named after the reference implementation, but unverified and NOT exposed
  // in the app. Sent twice to a Mera Comfort — empty seat and occupied seat —
  // and acknowledged with status 0 both times, with no audible or visible
  // effect and no change in any of the eight system parameters. The Mera has
  // no bowl flush of its own (that is the wall plate, which is not on this
  // BLE service), so the name is probably wrong. It may be the manual
  // counterpart to the SYSTEM_FLUSH profile setting on other models.
  TRIGGER_FLUSH: 37,
