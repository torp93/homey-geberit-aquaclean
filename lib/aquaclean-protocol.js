...
// Asking for more than this in one request answers normally and then leaves the
// toilet unable to answer GetFilterStatus (0x59) at all. The state survives
// reconnecting, a different client and restarting the proxy; only cycling mains
// power on the WC clears it. Reported against this same firmware in
// jens62/geberit-aquaclean#44, where [0,1,2,3,4,5,6,7,12,13] triggers it: the
// meaningful ids run past the end of the first BLE frame into a continuation
// frame. Read anything above index 7 in a separate request.
const MAX_SYSTEM_PARAMETERS_PER_REQUEST = 8;

const buildSystemParameterPayload = parameterIds => {
  const ids = Array.from(parameterIds);
  // A RangeError here is the cheap failure. The expensive one is a toilet that
  // needs its mains switched off before it will report its filter again.
  if (ids.length > MAX_SYSTEM_PARAMETERS_PER_REQUEST) {
    throw new RangeError(
      `AquaClean requests are limited to ${MAX_SYSTEM_PARAMETERS_PER_REQUEST} system parameters. `
      + `Asking for ${ids.length} crosses the BLE frame boundary and stops GetFilterStatus `
      + 'responding until the toilet is power-cycled. Split the read into several requests.',
    );
  }
...
const decodeSystemParameters = (result, parameterIds) => {
  ...
  return {
    ...
    // Parameter 7 is the service state. Named in the lid diagnostics run of
    // 2026-08-13, where it was confirmed against this Mera Comfort and sat at 0
    // throughout. It was already being requested and then dropped on the floor
    // here, which is why the app could not tell that the lid calibration had
    // left the toilet in service mode.
    serviceState: parameters[7],
