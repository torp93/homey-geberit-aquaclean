...
class EsphomeApiClient extends EventEmitter {
  ...
  listEntities(timeoutMs = 8000) {
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        off();
        reject(new Error('ESPHome API: tidsavbrudd på ListEntities'));
      }, timeoutMs);
  ...
  waitForDevice(mac, { timeoutMs = 15000 } = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        off();
        reject(new Error(`ESPHome API: så ikke ${mac} innen ${timeoutMs} ms`));
      }, timeoutMs);
    });
  }
  ...
  gattRead(mac, handle, { timeoutMs = 8000 } = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        off();
        reject(new Error(`GATT-lesing på handle ${handle}: tidsavbrudd`));
      }, timeoutMs);
  ...
  gattReadDescriptor(mac, handle, { timeoutMs = 8000 } = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        off();
        reject(new Error(`GATT-deskriptorlesing på handle ${handle}: tidsavbrudd`));
      }, timeoutMs);
  ...
  gattNotify(mac, handle, enable = true, { timeoutMs = 8000 } = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        off();
        reject(new Error(`GATT notify på handle ${handle}: tidsavbrudd`));
      }, timeoutMs);
  ...
  gattWriteDescriptor(mac, handle, data, { timeoutMs = 8000 } = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        off();
        reject(new Error(`GATT-deskriptorskriving på handle ${handle}: tidsavbrudd`));
      }, timeoutMs);
  ...
  _gattError(fields) {
    const err = new Error(`GATT-feil på handle ${Number(fields[2] || 0)}: error=${Number(fields[3] || 0)}`);
    err.code = 'GATT_ERROR';
  ...
  async close() {
    ...
    this.unsubscribeAdvertisements();
  ...
  _onData(chunk) {
    ...
    for (;;) {
      ...
      if (this._rx[0] !== 0x00) {
        this.emit('error', new Error('ESPHome API: forventet plaintext-ramme (er kryptering slått på?)'));
