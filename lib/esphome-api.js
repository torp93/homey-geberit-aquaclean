'use strict';

// ESPHome native API-klient (plaintext) med BLE-proxy-støtte.
//
// Bygget for Homey: ingen eksterne avhengigheter, kun node:net og node:events.
// Plaintext-ramme: 0x00 <varint payload_len> <varint msg_type> <payload>
//
// API 1.14 har fjernet passord-autentisering — ConnectRequest besvares ikke
// i det hele tatt. Gå rett fra HelloResponse til bruk.

const net = require('node:net');
const { EventEmitter } = require('node:events');

const MSG = {
  HELLO_REQ: 1, HELLO_RES: 2,
  DISCONNECT_REQ: 5, DISCONNECT_RES: 6,
  PING_REQ: 7, PING_RES: 8,
  DEVICE_INFO_REQ: 9, DEVICE_INFO_RES: 10,
  LIST_ENTITIES_REQ: 11,
  LIST_SENSOR: 16, LIST_SWITCH: 17, LIST_DONE: 19,
  SUBSCRIBE_STATES_REQ: 20, SENSOR_STATE: 25,
  LIST_BUTTON: 61, BUTTON_COMMAND_REQ: 62,
  SUB_BLE_ADV_REQ: 66, BLE_ADV_RES: 67,
  BLE_DEVICE_REQ: 68, BLE_DEVICE_CONN_RES: 69,
  GATT_GET_SERVICES_REQ: 70, GATT_GET_SERVICES_RES: 71, GATT_GET_SERVICES_DONE: 72,
  GATT_READ_REQ: 73, GATT_READ_RES: 74,
  GATT_WRITE_REQ: 75,
  GATT_READ_DESCRIPTOR_REQ: 76, GATT_WRITE_DESCRIPTOR_REQ: 77,
  GATT_NOTIFY_REQ: 78, GATT_NOTIFY_DATA: 79,
  GATT_ERROR: 82, GATT_WRITE_RES: 83, GATT_NOTIFY_RES: 84,
  UNSUB_BLE_ADV_REQ: 87,
  BLE_RAW_ADV_RES: 93,
};

const BLE_REQUEST = {
  CONNECT: 0,
  DISCONNECT: 1,
  CONNECT_V3_WITH_CACHE: 4,
  CONNECT_V3_WITHOUT_CACHE: 5,
};

const ADDRESS_TYPE_PUBLIC = 0;

// Largest plausible ESPHome native-API frame. The biggest thing this client
// ever receives is a GATT service list, a few kilobytes at most; 64 KiB leaves
// generous headroom while still catching a corrupted length varint.
const MAX_FRAME_BYTES = 64 * 1024;

// --- protobuf-primitiver -----------------------------------------------------

function encodeVarint(value) {
  let x = BigInt(value);
  if (x < 0n) throw new RangeError('varint cannot be negative');
  const out = [];
  do {
    let byte = Number(x & 0x7fn);
    x >>= 7n;
    if (x > 0n) byte |= 0x80;
    out.push(byte);
  } while (x > 0n);
  return Buffer.from(out);
}

function decodeVarint(buf, offset) {
  let result = 0n;
  let shift = 0n;
  let pos = offset;
  while (pos < buf.length) {
    const byte = buf[pos++];
    result |= BigInt(byte & 0x7f) << shift;
    if ((byte & 0x80) === 0) return { value: result, offset: pos };
    shift += 7n;
    if (shift > 70n) throw new Error('varint is too long');
  }
  return null; // ufullstendig
}

const tag = (field, wire) => encodeVarint((field << 3) | wire);
const pbVarint = (field, value) => Buffer.concat([tag(field, 0), encodeVarint(value)]);
const pbBool = (field, value) => pbVarint(field, value ? 1 : 0);
const pbFixed32 = (field, value) => {
  const b = Buffer.alloc(4);
  b.writeUInt32LE(value >>> 0);
  return Buffer.concat([tag(field, 5), b]);
};
const pbBytes = (field, data) => {
  const b = Buffer.from(data);
  return Buffer.concat([tag(field, 2), encodeVarint(b.length), b]);
};
const pbString = (field, str) => pbBytes(field, Buffer.from(str, 'utf8'));

// Gjentatte felter samles i array. uint64 beholdes som BigInt — BLE-adresser og
// 128-bits UUID-er overstiger Number.MAX_SAFE_INTEGER.
function decodeFields(buf) {
  const fields = {};
  const put = (no, val) => {
    if (fields[no] === undefined) fields[no] = val;
    else if (Array.isArray(fields[no])) fields[no].push(val);
    else fields[no] = [fields[no], val];
  };

  let pos = 0;
  while (pos < buf.length) {
    const key = decodeVarint(buf, pos);
    if (!key) break;
    pos = key.offset;
    const no = Number(key.value >> 3n);
    const wire = Number(key.value & 7n);

    if (wire === 0) {
      const v = decodeVarint(buf, pos);
      if (!v) break;
      put(no, v.value);
      pos = v.offset;
    } else if (wire === 2) {
      const len = decodeVarint(buf, pos);
      if (!len) break;
      pos = len.offset;
      const n = Number(len.value);
      put(no, buf.subarray(pos, pos + n));
      pos += n;
    } else if (wire === 5) {
      put(no, buf.readUInt32LE(pos));
      fields[`${no}_float`] = buf.readFloatLE(pos);
      pos += 4;
    } else if (wire === 1) {
      put(no, buf.readBigUInt64LE(pos));
      pos += 8;
    } else {
      break;
    }
  }
  return fields;
}

const asArray = (v) => (v === undefined ? [] : Array.isArray(v) ? v : [v]);

// Hver lytter ser ALLE innkommende meldinger, også annonseringsstrømmen der
// felt 1 er en liste med meldinger i stedet for en adresse. Sjekk derfor
// meldingstypen først — ellers kaster BigInt() på hver eneste annonsering.
const isReplyFor = (type, fields, address, ...expectedTypes) => {
  if (!expectedTypes.includes(type)) return false;
  return typeof fields[1] === 'bigint' && fields[1] === address;
};
const zigzag = (v) => { const x = BigInt(v); return Number((x >> 1n) ^ -(x & 1n)); };

const macToAddress = (mac) => BigInt('0x' + String(mac).split(':').join(''));
const addressToMac = (addr) =>
  BigInt(addr).toString(16).padStart(12, '0').match(/.{2}/g).join(':').toUpperCase();

// Tolker rå BLE-annonseringsdata: en sekvens av [lengde][type][data ...].
// Vi trenger navnet (0x08/0x09) og tjeneste-UUID-ene (0x02-0x07) for at
// driveren skal kjenne igjen enheten under paring.
function parseAdvertisementData(buffer) {
  const data = Buffer.from(buffer);
  const serviceUuids = [];
  let localName = '';
  let pos = 0;

  while (pos < data.length) {
    const length = data[pos];
    if (length === 0 || pos + length >= data.length + 1) break;
    const adType = data[pos + 1];
    const body = data.subarray(pos + 2, pos + 1 + length);

    if (adType === 0x08 || adType === 0x09) {
      localName = body.toString('utf8');
    } else if (adType === 0x02 || adType === 0x03) {
      for (let i = 0; i + 1 < body.length; i += 2) {
        // Homey oppgir alltid full 128-bits form, så vi utvider korte UUID-er
        // med Bluetooth Base UUID. Ellers bommer driverens tjenestesjekk.
        const short = body.readUInt16LE(i).toString(16).padStart(4, '0');
        serviceUuids.push(`0000${short}00001000800000805f9b34fb`);
      }
    } else if (adType === 0x06 || adType === 0x07) {
      for (let i = 0; i + 15 < body.length; i += 16) {
        // 128-bits UUID-er ligger little-endian på lufta.
        serviceUuids.push(Buffer.from(body.subarray(i, i + 16)).reverse().toString('hex'));
      }
    }
    pos += length + 1;
  }

  return { localName, serviceUuids };
}

function formatUuid(parts) {
  const list = asArray(parts);
  if (!list.length) return '';
  const hi = BigInt(list[0]).toString(16).padStart(16, '0');
  const lo = BigInt(list[1] !== undefined ? list[1] : 0).toString(16).padStart(16, '0');
  return hi + lo;
}

// --- klient ------------------------------------------------------------------

class EsphomeApiClient extends EventEmitter {
  constructor({ host, port = 6053, clientInfo = 'homey-aquaclean', log = () => {} } = {}) {
    super();
    if (!host) throw new TypeError('EsphomeApiClient requires a host');
    this.host = host;
    this.port = port;
    this.clientInfo = clientInfo;
    this._log = log;

    this._socket = null;
    this._rx = Buffer.alloc(0);
    this._waiters = [];
    this._connected = false;
    this._advSubscribed = false;
    this._bleConnected = new Set();   // BigInt-adresser
    this._closing = false;
  }

  get isConnected() { return this._connected; }
  get hasAdvertisementSubscription() { return this._advSubscribed; }

  connect({ timeoutMs = 10000 } = {}) {
    return new Promise((resolve, reject) => {
      const socket = net.createConnection({ host: this.host, port: this.port });
      this._socket = socket;

      const timer = setTimeout(() => {
        socket.destroy();
        reject(new Error(`ESPHome proxy at ${this.host}:${this.port} did not answer`));
      }, timeoutMs);

      // .on, ikke .once: en socketfeil ETTER tilkobling ville ellers vært en
      // unhandled 'error'-event som feller hele appen. Kun første feil skal
      // avvise connect-løftet.
      let rejected = false;
      socket.on('error', (err) => {
        clearTimeout(timer);
        this._failAll(err);
        if (!rejected) { rejected = true; reject(err); }
      });

      socket.on('data', (chunk) => this._onData(chunk));
      socket.on('close', () => {
        this._connected = false;
        this._advSubscribed = false;
        this._bleConnected.clear();
        this._failAll(new Error('ESPHome API: forbindelsen ble lukket'));
        this.emit('close');
      });

      socket.once('connect', async () => {
        try {
          const hello = await this._request(
            MSG.HELLO_REQ,
            Buffer.concat([pbString(1, this.clientInfo), pbVarint(2, 1), pbVarint(3, 9)]),
            MSG.HELLO_RES,
            timeoutMs,
          );
          clearTimeout(timer);
          this._connected = true;
          const info = {
            apiVersion: `${hello[1]}.${hello[2]}`,
            serverInfo: hello[3] ? hello[3].toString('utf8') : '',
            name: hello[4] ? hello[4].toString('utf8') : '',
          };
          this._log('esphome connected', info);
          resolve(info);
        } catch (err) {
          clearTimeout(timer);
          reject(err);
        }
      });
    });
  }

  async deviceInfo(timeoutMs = 8000) {
    const f = await this._request(MSG.DEVICE_INFO_REQ, Buffer.alloc(0), MSG.DEVICE_INFO_RES, timeoutMs);
    const str = (v) => (v ? v.toString('utf8') : '');
    return {
      name: str(f[2]),
      macAddress: str(f[3]),
      esphomeVersion: str(f[4]),
      model: str(f[6]),
      friendlyName: str(f[13]),
      bluetoothProxyFeatureFlags: Number(f[15] || 0),
    };
  }

  // Samler entiteter til ListEntitiesDone. Knappenøkler trengs for restart.
  listEntities(timeoutMs = 8000) {
    return new Promise((resolve, reject) => {
      const sensors = new Map();
      const buttons = new Map();
      const switches = new Map();

      const off = this._collect((type, fields) => {
        if (type === MSG.LIST_SENSOR) {
          sensors.set(Number(fields[2]), {
            key: Number(fields[2]),
            name: fields[3] ? fields[3].toString('utf8') : '',
            unit: fields[6] ? fields[6].toString('utf8') : '',
          });
        } else if (type === MSG.LIST_BUTTON) {
          buttons.set(fields[3] ? fields[3].toString('utf8') : '', Number(fields[2]));
        } else if (type === MSG.LIST_SWITCH) {
          switches.set(fields[3] ? fields[3].toString('utf8') : '', Number(fields[2]));
        } else if (type === MSG.LIST_DONE) {
          off();
          clearTimeout(timer);
          resolve({ sensors, buttons, switches });
        }
      });

      const timer = setTimeout(() => {
        off();
        reject(new Error('ESPHome proxy did not answer ListEntities'));
      }, timeoutMs);

      this._send(MSG.LIST_ENTITIES_REQ, Buffer.alloc(0));
    });
  }

  subscribeStates() {
    this._send(MSG.SUBSCRIBE_STATES_REQ, Buffer.alloc(0));
  }

  // Brukes av kretsbryteren: trykk "Restart AquaClean Proxy" når BLE henger.
  pressButton(key) {
    this._send(MSG.BUTTON_COMMAND_REQ, pbFixed32(1, key));
  }

  subscribeAdvertisements() {
    if (this._advSubscribed) return;
    // ESP32 tillater KUN ett advertisement-abonnement om gangen, på tvers av
    // alle TCP-forbindelser. Vi sporer det selv for ikke å be om to.
    this._send(MSG.SUB_BLE_ADV_REQ, pbVarint(1, 0));
    this._advSubscribed = true;
  }

  unsubscribeAdvertisements() {
    if (!this._advSubscribed) return;
    this._send(MSG.UNSUB_BLE_ADV_REQ, Buffer.alloc(0));
    this._advSubscribed = false;
  }

  // Venter på at en gitt adresse dukker opp i advertisements.
  waitForDevice(mac, { timeoutMs = 15000 } = {}) {
    const target = macToAddress(mac);
    return new Promise((resolve, reject) => {
      const off = this._collect((type, fields, raw) => {
        const seen = this._addressesFromAdvertisement(type, fields, raw);
        for (const entry of seen) {
          if (entry.address === target) {
            off();
            clearTimeout(timer);
            resolve(entry);
            return;
          }
        }
      });
      const timer = setTimeout(() => {
        off();
        reject(new Error(`Did not see ${mac} within ${timeoutMs} ms`));
      }, timeoutMs);
    });
  }

  _addressesFromAdvertisement(type, fields) {
    if (type === MSG.BLE_ADV_RES) {
      if (fields[1] === undefined) return [];
      return [{
        address: BigInt(fields[1]),
        rssi: zigzag(fields[3] || 0),
        localName: fields[2] ? Buffer.from(fields[2]).toString('utf8') : '',
        serviceUuids: asArray(fields[4]).map((u) => Buffer.from(u).toString('utf8')),
      }];
    }
    if (type === MSG.BLE_RAW_ADV_RES) {
      return asArray(fields[1]).map((buf) => {
        const a = decodeFields(buf);
        // Rå annonsering leveres som AD-strukturer og må tolkes her — uten
        // navn og tjeneste-UUID-er kan driveren ikke kjenne igjen toalettet.
        const parsed = a[4] ? parseAdvertisementData(a[4]) : { localName: '', serviceUuids: [] };
        return {
          address: BigInt(a[1] || 0),
          rssi: zigzag(a[2] || 0),
          localName: parsed.localName,
          serviceUuids: parsed.serviceUuids,
        };
      });
    }
    return [];
  }

  async bleConnect(mac, { addressType = ADDRESS_TYPE_PUBLIC, timeoutMs = 25000 } = {}) {
    const address = macToAddress(mac);
    const payload = Buffer.concat([
      pbVarint(1, address),
      pbVarint(2, BLE_REQUEST.CONNECT_V3_WITHOUT_CACHE),
      pbBool(3, true),
      pbVarint(4, addressType),
    ]);

    return new Promise((resolve, reject) => {
      const off = this._collect((type, fields) => {
        if (!isReplyFor(type, fields, address, MSG.BLE_DEVICE_CONN_RES)) return;
        off();
        clearTimeout(timer);
        if (Number(fields[2] || 0) === 1) {
          this._bleConnected.add(address);
          resolve({ mtu: Number(fields[3] || 0) });
        } else {
          reject(new Error(`Bluetooth connection refused (error=${Number(fields[4] || 0)})`));
        }
      });
      const timer = setTimeout(() => {
        off();
        reject(new Error(`Bluetooth connection timed out after ${timeoutMs} ms`));
      }, timeoutMs);
      this._send(MSG.BLE_DEVICE_REQ, payload);
    });
  }

  bleDisconnect(mac) {
    const address = macToAddress(mac);
    this._bleConnected.delete(address);
    this._send(MSG.BLE_DEVICE_REQ, Buffer.concat([
      pbVarint(1, address),
      pbVarint(2, BLE_REQUEST.DISCONNECT),
    ]));
  }

  getServices(mac, { timeoutMs = 20000 } = {}) {
    const address = macToAddress(mac);
    return new Promise((resolve, reject) => {
      const services = [];
      const off = this._collect((type, fields) => {
        if (!isReplyFor(type, fields, address,
          MSG.GATT_GET_SERVICES_RES, MSG.GATT_GET_SERVICES_DONE)) return;
        if (type === MSG.GATT_GET_SERVICES_RES) {
          for (const svcBuf of asArray(fields[2])) {
            const s = decodeFields(svcBuf);
            services.push({
              uuid: formatUuid(s[1]),
              handle: Number(s[2] || 0),
              characteristics: asArray(s[3]).map((chBuf) => {
                const c = decodeFields(chBuf);
                const descriptors = asArray(c[4]).map((dBuf) => {
                  const d = decodeFields(dBuf);
                  return { uuid: formatUuid(d[1]), handle: Number(d[2] || 0) };
                });
                // 0x2902 = Client Characteristic Configuration Descriptor.
                const cccd = descriptors.find((d) => d.uuid.includes('2902'));
                return {
                  uuid: formatUuid(c[1]),
                  handle: Number(c[2] || 0),
                  properties: Number(c[3] || 0),
                  descriptors,
                  cccdHandle: cccd ? cccd.handle : null,
                };
              }),
            });
          }
        } else if (type === MSG.GATT_GET_SERVICES_DONE) {
          off();
          clearTimeout(timer);
          resolve(services);
        }
      });
      const timer = setTimeout(() => {
        off();
        reject(new Error('GATT service discovery timed out'));
      }, timeoutMs);
      this._send(MSG.GATT_GET_SERVICES_REQ, pbVarint(1, address));
    });
  }

  gattRead(mac, handle, { timeoutMs = 8000 } = {}) {
    const address = macToAddress(mac);
    return new Promise((resolve, reject) => {
      const off = this._collect((type, fields) => {
        if (!isReplyFor(type, fields, address, MSG.GATT_READ_RES, MSG.GATT_ERROR)) return;
        if (Number(fields[2] || 0) !== handle) return;
        off(); clearTimeout(timer);
        if (type === MSG.GATT_ERROR) reject(this._gattError(fields));
        else resolve(fields[3] ? Buffer.from(fields[3]) : Buffer.alloc(0));
      });
      const timer = setTimeout(() => {
        off();
        reject(new Error(`GATT read on handle ${handle} timed out`));
      }, timeoutMs);
      this._send(MSG.GATT_READ_REQ, Buffer.concat([pbVarint(1, address), pbVarint(2, handle)]));
    });
  }

  // Leser en deskriptor (f.eks. CCCD) — samme svartype som vanlig lesing.
  gattReadDescriptor(mac, handle, { timeoutMs = 8000 } = {}) {
    const address = macToAddress(mac);
    return new Promise((resolve, reject) => {
      const off = this._collect((type, fields) => {
        if (!isReplyFor(type, fields, address, MSG.GATT_READ_RES, MSG.GATT_ERROR)) return;
        if (Number(fields[2] || 0) !== handle) return;
        off(); clearTimeout(timer);
        if (type === MSG.GATT_ERROR) reject(this._gattError(fields));
        else resolve(fields[3] ? Buffer.from(fields[3]) : Buffer.alloc(0));
      });
      const timer = setTimeout(() => {
        off();
        reject(new Error(`GATT descriptor read on handle ${handle} timed out`));
      }, timeoutMs);
      this._send(MSG.GATT_READ_DESCRIPTOR_REQ, Buffer.concat([pbVarint(1, address), pbVarint(2, handle)]));
    });
  }

  // withResponse=false gir ATT_WRITE_COMMAND. Geberit sine skrivekanaler har
  // både write og write-no-resp; vi bruker write-med-svar som standard slik at
  // en feilet skriving faktisk rapporteres.
  gattWrite(mac, handle, data, { withResponse = true, timeoutMs = 8000 } = {}) {
    const address = macToAddress(mac);
    const payload = Buffer.concat([
      pbVarint(1, address),
      pbVarint(2, handle),
      pbBool(3, withResponse),
      pbBytes(4, data),
    ]);

    if (!withResponse) {
      this._send(MSG.GATT_WRITE_REQ, payload);
      return Promise.resolve();
    }

    return new Promise((resolve, reject) => {
      const off = this._collect((type, fields) => {
        if (!isReplyFor(type, fields, address, MSG.GATT_WRITE_RES, MSG.GATT_ERROR)) return;
        if (Number(fields[2] || 0) !== handle) return;
        off(); clearTimeout(timer);
        if (type === MSG.GATT_ERROR) reject(this._gattError(fields));
        else resolve();
      });
      const timer = setTimeout(() => {
        off();
        reject(new Error(`GATT write on handle ${handle} timed out`));
      }, timeoutMs);
      this._send(MSG.GATT_WRITE_REQ, payload);
    });
  }

  gattNotify(mac, handle, enable = true, { timeoutMs = 8000 } = {}) {
    const address = macToAddress(mac);
    const payload = Buffer.concat([pbVarint(1, address), pbVarint(2, handle), pbBool(3, enable)]);
    return new Promise((resolve, reject) => {
      const off = this._collect((type, fields) => {
        if (!isReplyFor(type, fields, address, MSG.GATT_NOTIFY_RES, MSG.GATT_ERROR)) return;
        if (Number(fields[2] || 0) !== handle) return;
        off(); clearTimeout(timer);
        if (type === MSG.GATT_ERROR) reject(this._gattError(fields));
        else resolve();
      });
      const timer = setTimeout(() => {
        off();
        reject(new Error(`GATT notify on handle ${handle} timed out`));
      }, timeoutMs);
      this._send(MSG.GATT_NOTIFY_REQ, payload);
    });
  }

  gattWriteDescriptor(mac, handle, data, { timeoutMs = 8000 } = {}) {
    const address = macToAddress(mac);
    const payload = Buffer.concat([pbVarint(1, address), pbVarint(2, handle), pbBytes(3, data)]);
    return new Promise((resolve, reject) => {
      const off = this._collect((type, fields) => {
        if (!isReplyFor(type, fields, address, MSG.GATT_WRITE_RES, MSG.GATT_ERROR)) return;
        if (Number(fields[2] || 0) !== handle) return;
        off(); clearTimeout(timer);
        if (type === MSG.GATT_ERROR) reject(this._gattError(fields));
        else resolve();
      });
      const timer = setTimeout(() => {
        off();
        reject(new Error(`GATT descriptor write on handle ${handle} timed out`));
      }, timeoutMs);
      this._send(MSG.GATT_WRITE_DESCRIPTOR_REQ, payload);
    });
  }

  // ESPHome sin BluetoothGATTNotifyRequest registrerer kun callbacken lokalt på
  // ESP32-en — den skriver IKKE CCCD-deskriptoren på enheten. Bekreftet mot ekte
  // maskinvare: CCCD stod på 0x0000 etter en kvittert notify-forespørsel, og
  // toalettet sendte følgelig ingenting. Vi må skrive 0x0001 selv.
  async enableNotifications(mac, characteristic) {
    await this.gattNotify(mac, characteristic.handle, true);
    const cccd = characteristic.cccdHandle ?? characteristic.handle + 1;
    await this.gattWriteDescriptor(mac, cccd, Buffer.from([0x01, 0x00]));
    return cccd;
  }

  _gattError(fields) {
    const err = new Error(`GATT error on handle ${Number(fields[2] || 0)}: error=${Number(fields[3] || 0)}`);
    err.code = 'GATT_ERROR';
    err.attError = Number(fields[3] || 0);
    return err;
  }

  async close() {
    this._closing = true;
    // Rekkefølgen er kritisk: BLE ned FØRST, deretter unsubscribe, så TCP.
    // Motsatt rekkefølge river ned BLE-linken og etterlater abonnementet
    // hengende på ESP32-en til ping-timeout (~90 s).
    // Every step here writes to the socket, and _send throws once the peer has
    // destroyed it — an ESP32 that reboots mid-shutdown used to make close()
    // reject before the socket was destroyed, leaving it open forever with
    // _closing stuck true. Shutdown is best-effort by nature: the tidy exit is
    // worth attempting, but never at the cost of the teardown below it.
    const attempt = (what, action) => {
      try {
        action();
      } catch (error) {
        this._log('esphome close step failed', { step: what, error: error.message });
      }
    };

    for (const address of [...this._bleConnected]) {
      attempt('bleDisconnect', () => this.bleDisconnect(addressToMac(address)));
    }
    if (this._bleConnected.size) await new Promise((r) => setTimeout(r, 400));
    attempt('unsubscribeAdvertisements', () => this.unsubscribeAdvertisements());
    await new Promise((r) => setTimeout(r, 200));
    if (this._socket && !this._socket.destroyed) {
      attempt('disconnectRequest', () => this._send(MSG.DISCONNECT_REQ, Buffer.alloc(0)));
      await new Promise((r) => setTimeout(r, 200));
    }
    if (this._socket && !this._socket.destroyed) this._socket.destroy();
    this._connected = false;
  }

  // --- internt ---------------------------------------------------------------

  _send(type, payload) {
    if (!this._socket || this._socket.destroyed) {
      throw new Error('ESPHome proxy is not connected');
    }
    this._socket.write(Buffer.concat([
      Buffer.from([0x00]),
      encodeVarint(payload.length),
      encodeVarint(type),
      payload,
    ]));
  }

  _request(type, payload, expectType, timeoutMs) {
    return new Promise((resolve, reject) => {
      const off = this._collect((t, fields) => {
        if (t !== expectType) return;
        off();
        clearTimeout(timer);
        resolve(fields);
      });
      const timer = setTimeout(() => {
        off();
        reject(new Error(`ESPHome proxy did not answer message type ${expectType}`));
      }, timeoutMs);
      this._send(type, payload);
    });
  }

  _collect(handler) {
    this._waiters.push(handler);
    return () => {
      const i = this._waiters.indexOf(handler);
      if (i >= 0) this._waiters.splice(i, 1);
    };
  }

  _failAll(error) {
    // Waiters rydder seg selv via sine timeouts; her varsler vi bare oppover.
    // Planlagt nedkobling er ikke en feil og skal ikke støye.
    if (this._closing) return;
    this.emit('error', error);
  }

  _onData(chunk) {
    this._rx = Buffer.concat([this._rx, chunk]);

    for (;;) {
      if (this._rx.length < 1) return;
      if (this._rx[0] !== 0x00) {
        this.emit('error', new Error('Expected a plaintext frame from the ESPHome proxy (is encryption enabled?)'));
        if (this._socket && !this._socket.destroyed) this._socket.destroy();
        return;
      }
      const lenV = decodeVarint(this._rx, 1);
      if (!lenV) return;
      const typeV = decodeVarint(this._rx, lenV.offset);
      if (!typeV) return;
      const end = typeV.offset + Number(lenV.value);
      // A corrupted length varint can claim a frame far larger than anything
      // ESPHome sends. Without a ceiling the loop below would wait for bytes
      // that never come while every later chunk is concatenated onto _rx —
      // unbounded growth on a Homey that is already short of memory, with no
      // way back into sync. Resynchronising is not possible mid-stream, so the
      // socket is dropped and the normal reconnect ladder takes over.
      if (Number(lenV.value) > MAX_FRAME_BYTES) {
        this.emit('error', new Error(
          `ESPHome API: frame claims ${lenV.value} bytes, refusing to buffer it`,
        ));
        if (this._socket && !this._socket.destroyed) this._socket.destroy();
        return;
      }
      if (this._rx.length < end) return;

      const payload = this._rx.subarray(typeV.offset, end);
      this._rx = this._rx.subarray(end);
      const type = Number(typeV.value);

      let fields;
      try {
        fields = decodeFields(payload);
      } catch (err) {
        this._log('esphome decode failed', { type, error: err.message });
        continue;
      }

      // _dispatch reads wire fields positionally — addressToMac() and BigInt()
      // both throw if a field arrives as a Buffer rather than a varint. Outside
      // a try/catch that throw travels up through the socket's 'data' handler
      // and takes the whole app down. The frame is already consumed above, so
      // dropping this one message and carrying on keeps framing intact.
      try {
        this._dispatch(type, fields, payload);
      } catch (err) {
        this._log('esphome dispatch failed', { type, error: err.message });
      }
    }
  }

  _dispatch(type, fields, payload) {
    // Rå innsyn for feilsøking av meldingstyper mot ekte maskinvare.
    this.emit('message', { type, fields, payload });

    if (type === MSG.PING_REQ) {
      this._send(MSG.PING_RES, Buffer.alloc(0));
      return;
    }

    if (type === MSG.GATT_NOTIFY_DATA) {
      this.emit('notify', {
        mac: addressToMac(fields[1] || 0),
        handle: Number(fields[2] || 0),
        data: fields[3] ? Buffer.from(fields[3]) : Buffer.alloc(0),
      });
    } else if (type === MSG.BLE_DEVICE_CONN_RES && Number(fields[2] || 0) === 0) {
      const address = BigInt(fields[1] || 0);
      if (this._bleConnected.delete(address)) {
        this.emit('bleDisconnected', {
          mac: addressToMac(address),
          error: Number(fields[4] || 0),
        });
      }
    } else if (type === MSG.SENSOR_STATE) {
      this.emit('sensorState', {
        key: Number(fields[1] || 0),
        value: fields['2_float'],
        missing: Boolean(fields[3]),
      });
    }

    for (const handler of [...this._waiters]) {
      try {
        handler(type, fields, payload);
      } catch (err) {
        this._log('esphome handler failed', err.message);
      }
    }
  }
}

module.exports = {
  EsphomeApiClient,
  MSG,
  BLE_REQUEST,
  addressToMac,
  macToAddress,
  decodeFields,
  decodeVarint,
  encodeVarint,
  formatUuid,
  parseAdvertisementData,
};
