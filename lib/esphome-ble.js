...
class EsphomePeripheral extends EventEmitter {
  ...
  _assertConnected() {
    if (!this.isConnected) {
      const error = new Error('BLE-enheten er ikke tilkoblet.');
...
class EsphomeBle {
  constructor({ host, port = 6053, log = () => {} } = {}) {
    if (!host) throw new TypeError('EsphomeBle krever host til proxyen');
    this.host = host;
    this.port = port;
    this._log = log;
    this._client = null;
  ...
  async _ensureClient() {
    ...
    this._connecting = (async () => {
        ...
        for (const peripheral of [...this._peripherals]) {
          if (peripheral.address === mac) {
            ...
            peripheral._handleDisconnect(
              error ? new Error(`BLE-forbindelsen falt (error=${error})`) : undefined,
            );
          }
        }
      });
      ...
      client.on('close', () => {
        for (const peripheral of [...this._peripherals]) {
          this._peripherals.delete(peripheral);
          peripheral._handleDisconnect(new Error('Forbindelsen til BLE-proxyen falt.'));
        }
        this._client = null;
      });
